...
//var app = angular.module('icpsIcpo', [ 'chieffancypants.loadingBar', 'ngAnimate' ]);
...
//function factSheetController($scope, $http, cfpLoadingBar){
    ...
    then(function(result){
    	...
    	/*$timeout(function(){
			if($(".table-header-fixed")[0].offsetWidth >= $(".table-header-fixed")[0].clientWidth){
				var i=0;rowWidthList=[];
				$(".static-header-container .static-header").html($(".table-header-fixed").html());
				$(".table-header-fixed thead tr").find("th").each(function(){
					rowWidthList.push($(this)[0].offsetWidth);
				});
				$(".static-header thead tr").find("th").each(function(){
					$(this).width(rowWidthList[i++]);
				});
				$(".static-header-container").height($(".static-header table thead").height());
				$(".table-header-fixed").css("margin-top", -$(".static-header table thead").height()+"px");
...
/*    		$scope.KeysOrder=$filter('orderBy')($scope.Keys, ['keyValue']);*/
    		//$scope.Keys=Object.keys($scope.subSectorData[0].sectorData);
    		/*for(i=0 ; i<$scope.selectedSector.children.length;i++)
    			{	
    				for(j=0;j<$scope.selectedSector.children[i].children.length;j++)
    					{
    						for(k=0;k<$scope.subSectorData.length;k++)
    							{
    								if($scope.subSectorData[i].sectorData[$scope.selectedSector.children[i].children.indicator_Name]==undefined)
    									{
    									var key= $scope.selectedSector.children[i].children.indicator_Name;
    									$scope.subSectorData[i].sectorData.key="N/A";
    									}
...
//    function rebuildProgramMultiSelect(){
//		$('#districtSelection').empty();
//		$("#districtSelection option:selected").removeAttr("selected");
//		for(var k = 0; k < $scope.districtList.length; k++){
//			$('#districtSelection').append($('<option>', {
//				value : $scope.districtList[k].area_NId,
//				text : $scope.districtList[k].area_Name
//			}));
//		}	
//		$('#districtSelection').multiselect('rebuild');
//	}
