...
import org.sdrc.scpsassam.exceptions.UserNotFoundException;
import org.sdrc.scpsassam.model.UserModel;
import org.sdrc.scpsassam.repository.AreaRepository;
import org.sdrc.scpsassam.repository.UserLoginMetaRepository;
import org.sdrc.scpsassam.repository.UserRepository;
import org.sdrc.scpsassam.util.DomainToModelConverter;
	...
	public boolean forgotPassword(String username) throws MessagingException {
		...
		new Thread() {
			...
			public void run() {
				...
				String userName = applicationMessageSource.getMessage("forgot.pass.mail", null, null);
				String password = applicationMessageSource.getMessage("forgot.pass.password", null, null);
				...
				return new PasswordAuthentication(
				  ...
				  try{  
					...
					String messageBody="<html><body><p style='color:#2d3e50; font-size:14px; font-weight:normal; margin:10px 0'>"
							+ "Please <span>find below</span> credentials for accessing the SCPS application.<br>"
							+ "Username: " + user.getUserName() + "<br> OTP: " + new String(otp) +"</p>"
							+ "<br><p> OTP: " + new String(otp) + "</p>"
							+ "<p style='color:#2d3e50; font-size:14px; font-weight:normal; margin:10px 0'>Please  <span >note:</span> We would strongly recommend you to create a new password. </p>"
							+ "</body></html>";
